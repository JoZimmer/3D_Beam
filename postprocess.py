...
class Postprocess(object):
    ...
    def plot_static_result(self,beam_model, load_type = 'single' ,dofs_to_plot = ['y'], analytic = True):
    ...
    def plot_objective_function_3D(self,optimization_object, evaluation_space_x = [-4,4,0.1], evaluation_space_y = None,
                                    include_opt_history = False, fig_label = '', fifilename_for_save ='0_no_name', add_3D_surf_plot = False):
        '''
        be careful when this is called and what the attributes in the beam object are at this time 
        target ?! correct?
        ...
        ax1.set_title(r'optimizable function with weights: ' + r'${}$'.format(optimization_object.weights))
        ...
        k_ya_hist = optimization_object.optimization_history['k_ya'].copy()
        k_ga_hist = optimization_object.optimization_history['k_ga'].copy()
        func_hist = optimization_object.optimization_history['func'].copy()
        ...
        if include_opt_history:
            ax1.plot(k_ya_hist, 
                    k_ga_hist, 
                    linestyle = '--', color = 'grey',  # line stuff
                    marker = 'o', mfc = 'grey', mec='k', ms = 3,  # marker stuff
                    label='iterations')
            ax1.scatter(k_ya_hist[0],k_ga_hist[0], marker='o',c='g',edgecolors='k', s = 80,label='start',zorder=5)
            ax1.scatter(k_ya_hist[-1],k_ga_hist[-1], marker='o',c='m',edgecolors='k', s=80, label='end',zorder=5)
        ...
        cs_bar.ax.set_xlabel(r'$ f = \sum^{3} w_{i} * r_{i} ^{2}$')
        ax1.grid( linestyle='--')
        ax1.legend()
    ...
    def plot_optimization_history(self, optimization_object, include_func, norm_with_final = True):

        fig = plt.figure(num='opt_history')
        ...
        for key, val in optimization_object.optimization_history.items():
            ...
            if key == 'iter':
                continue
            if norm_with_final:
                val = [val_i / val[0] for val_i in val]
            plt.plot(np.arange(1,len(val)+1), val, label=utilities.prepare_string_for_latex(key))
            plt.xlabel('Iteration')
            plt.ylabel('x / x_start')
            plt.grid()
        plt.legend()
        plt.show()
...
# # DYNAMIC ANALYSIS
    ...
    def plot_dynamic_results(self, dynamic_analysis, dof_label, node_id, result_variable):
        ...
        if result_variable == 'displacement':
            ...
            if dof in dynamic_analysis.structure_model.dofs_of_bc:# or dof in dynamic_analysis.structure_model.elastic_bc_dofs:
                ...
                err_msg += "Choose one of: " + \
                           ", ".join([str(val)
                                      for val in dynamic_analysis.structure_model.bc_dofs])
                raise Exception(err_msg)

        fig = plt.figure(num='dynamic_result')
        ...
        # plot undeformed
        plt.plot(dynamic_analysis.array_time,
                result_data)
        #ax.legend()
